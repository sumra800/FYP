...
def ask_course_question(question: str, course_filter: str = None, max_context_chunks=30):
    """Query the course material RAG system with safe fallback.

    Behavior changes:
    - Request distances from the DB retrieval.
    - If retrieval returns no useful excerpts, call the LLM in fallback mode
      (unsourced answer) and clearly label the response.
    """
    ...
    # If results structure is unexpected, fallback early
    docs = results.get('documents', [[]])[0]
    metas = results.get('metadatas', [[]])[0]
    dists = results.get('distances', [[]])[0] if 'distances' in results else []

    # Build context parts from retrieved docs
    context_parts = []
    for doc, metadata in zip(docs, metas):
        if doc and doc.strip():
            context_parts.append(f"[{metadata.get('course', 'unknown')}] {doc}")

    # Basic relevance check: no useful docs
    if len(context_parts) == 0:
        # Fallback: ask the model directly but label as unsourced
        fallback_prompt = f"Provide a concise factual answer to: {question}\n\nIf you are not certain, say so."
        answer, err = send_prompt_to_api(fallback_prompt, max_tokens=400)
        if err:
            return f"Error querying API: {err}", []
        labeled = "[UNSOURCED ANSWER — based on general knowledge]\n" + answer
        return labeled, []

    # Optional: check best distance (if distances present) and decide fallback
    try:
        # For many vector stores lower distance = more similar. We treat high distances as low confidence.
        if dists:
            best_dist = min(dists)
            # If best distance is large (threshold may need tuning), fallback.
            # Use a conservative threshold here (e.g., 0.9 for cosine distance) — adjust per your DB.
            if best_dist > 0.9:
                fallback_prompt = f"Provide a concise factual answer to: {question}\n\nIf you are not certain, say so."
                answer, err = send_prompt_to_api(fallback_prompt, max_tokens=400)
                if err:
        ...
        # ignore errors in distance logic and continue
        pass

    # Deduplicate near-identical text (keep order)
    unique_contexts = list(dict.fromkeys(context_parts))
    context = "\n\n".join(unique_contexts)

    # Craft RAG prompt (strict: only use excerpts)
    rag_prompt = f"""
You are an expert teaching assistant helping a student understand their course material.

Based EXCLUSIVELY on the following excerpts from the course textbook, please answer the student's question.

If the question cannot be answered from the provided context, please say so rather than making up an answer.
...
""".strip()
    ...
    try:
        ...
        source_courses = list(set([m.get('course') for m in metas if isinstance(m, dict) and 'course' in m]))

        # If assistant explicitly says it cannot answer, optionally perform an unsourced fallback
        lower_answer = (answer or '').lower()
        if 'cannot' in lower_answer or 'cannot answer' in lower_answer or 'cannot be answered' in lower_answer:
            fallback_prompt = f"Provide a concise factual answer to: {question}\n\nIf you are not certain, say so."
            fb_ans, err = send_prompt_to_api(fallback_prompt, max_tokens=400)
            if err:
                # return the original (sourced) answer if fallback fails
                return answer, source_courses
            labeled = "[UNSOURCED ANSWER — based on general knowledge]\n" + fb_ans
            return labeled, source_courses
